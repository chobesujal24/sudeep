        ...
        {filteredProducts.length === 0 ? (
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-8">
            {filteredProducts.map((product, i) => (
              <div key={i} className="bg-[color:var(--color-bg-card)] border border-[color:var(--color-border)] shadow-sm rounded-2xl overflow-hidden flex flex-col group hover:shadow-2xl transition-all duration-500 hover:border-[color:var(--color-primary)]/40 hover:-translate-y-1.5 relative">
                {/* Image placeholder */}
                <div className="relative h-64 bg-white overflow-hidden flex items-center justify-center p-4">
                  {product.images?.[0] ? (
                    <Image 
                      ...
                      fill
                      unoptimized
                      className="object-contain p-4 opacity-90 group-hover:opacity-100 group-hover:scale-[1.03] transition-all duration-500"
                    />
                  ) : (
